...
export async function POST(req: NextRequest) {
  ...
  if (!checkRateLimit(token.id as string)) {
    await logAudit({ userId: token.id as string, action: "VALIDATE_RATE_LIMITED", entity: "checkin" });
  ...
  if (assignmentId) {
    ...
    await db.update(qrSessions).set({ consumedAt: new Date(), consumedBy: token.id as string })
      .where(and(eq(qrSessions.checkinId, checkin.id)));

    await logAudit({ userId: token.id as string, action: "CHECKIN_VALIDATED_APP", entity: "checkin", entityId: checkin.id });

    return NextResponse.json({ success: true, data: { method: "APP_DIRECT" } });
  }

  if (code) {
    return validateByCode(code, token.id as string);
  }

  return NextResponse.json({ success: false, error: "Informe assignmentId ou code" }, { status: 400 });
}

async function validateByCode(code: string, validatorId: string) {
  ...
  await logAudit({ userId: validatorId, action: "CHECKIN_VALIDATED_CODE", entity: "checkin", entityId: session.checkinId });
