...
const ruleSchema = z.object({
  ...
  capacity: z.number().int().min(0).max(10).default(1),
...
export async function POST(req: NextRequest) {
  ...
  const [created] = await db.insert(slotRules).values(parsed.data).returning();
  await logAudit({ userId: token.id as string, action: "CREATE_SLOT_RULE", entity: "slot_rule", entityId: created.id, payload: parsed.data });
...
export async function PUT(req: NextRequest) {
  ...
  const [updated] = await db.update(slotRules).set(parsed.data).where(eq(slotRules.id, id)).returning();
  if (!updated) return NextResponse.json({ success: false, error: "Regra não encontrada" }, { status: 404 });

  await logAudit({ userId: token.id as string, action: "UPDATE_SLOT_RULE", entity: "slot_rule", entityId: id, payload: parsed.data });
