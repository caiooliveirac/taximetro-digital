...
import { Sun, Moon, Plus, Loader2, Trash2, X, Save, LayoutGrid, TableProperties } from "lucide-react";
...
export default function AdminEscalas() {
  const [activeTab, setActiveTab] = useState<"filled" | "grade">("filled");
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Grade e Escala</h1>
          <p className="text-sm text-slate-500">A grade define vagas por faculdade. A aba escala compila a visão operacional completa da semana.</p>
        </div>
        ...
        <div className="inline-flex rounded-2xl border border-slate-200 bg-white p-1 shadow-sm">
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition ${activeTab === "grade" ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-50"}`}
          >
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition ${activeTab === "filled" ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-50"}`}
          >
            <TableProperties className="h-4 w-4" /> Escala
          </button>
        </div>
      </div>
