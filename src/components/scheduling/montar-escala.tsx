...
/** Vaga que OUTRA faculdade liberou e esta pode usar. */
type FreeSlot = {
  id: string; baseId: string; baseCode: string; baseType: string; date: string; period: string;
  shift: string | null; releasedFacultyAbbr: string;
};
type CruFixed = {
...
/**
 * Montar Escala — CRU fixo semanal + sorteio de intervenção nas bases USA.
 *
 * Serve duas telas com o mesmo código: `/leader/escala`, onde a faculdade vem do
 * vínculo do líder, e `/admin/montar-escala`, onde o coordenador escolhe a
 * faculdade e ela é passada em `facultyId`. As diferenças de rota, query, corpo
 * e cabeçalho saem todas de `escala-scope.ts`.
 */
export function MontarEscala({ facultyId }: { facultyId?: string | null } = {}) {
  const { data: session } = useSession();
  const { target: impersonateTarget } = useImpersonate();
  const escopo = useMemo(() => escopoDeEscala(facultyId), [facultyId]);
  const baseApi = baseDaEscala(escopo);
  // No admin a faculdade da alocação manual é a escolhida na tela; no líder,
  // a do vínculo (ou a do líder visitado por impersonate).
  const effectiveFacultyId = facultyId ?? impersonateTarget?.facultyId ?? session?.user?.facultyId;

  const fetchJsonNoStore = useCallback(async (url: string) => {
    const response = await fetch(urlComFaculdade(url, escopo), {
      cache: "no-store",
  ...
  /** POST/PATCH/DELETE desta tela: faculdade no corpo e cabeçalho do escopo. */
  const enviarJson = useCallback((url: string, method: string, corpo?: object) => {
    // Sem corpo (o DELETE do CRU fixo), a query é o único lugar que sobra.
    return fetch(corpo ? url : urlComFaculdade(url, escopo), {
      method,
      headers: cabecalhosDaEscala(escopo, { "Content-Type": "application/json" }),
      ...(corpo ? { body: JSON.stringify(corpoComFaculdade(corpo, escopo)) } : {}),
    });
  }, [escopo]);
  ...
  const [allocSlot, setAllocSlot] = useState<{
    baseId: string; baseCode: string; baseType: string; date: string; period: "DAY" | "NIGHT";
    /** Vaga livre de outra faculdade: a alocação passa pela oferta, não pela regra de vaga. */
    free?: { id: string; abbr: string };
  } | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [uRes, bRes, aRes, sRes, cfRes, rlRes, frRes] = await Promise.all([
        ...
        fetchJsonNoStore(`${baseApi}/released-slots?from=${from}&to=${to}&free=1`),
      ]);
      const [uJson, bJson, aJson, sJson, cfJson, rlJson, frJson] = await Promise.all([uRes, bRes, aRes, sRes, cfRes, rlRes, frRes]);
      ...
      if (frJson.success) setFreeSlots(frJson.data);
    } catch {
      setLotteryMsg("❌ Erro ao carregar dados da escala.");
    }
    setLoading(false);
  }, [baseApi, enviarJson, fetchJsonNoStore, weekDates, weekStart]);
  ...
  const freeBySlot = useMemo(() => {
    ...
    for (const f of freeSlots) {
      const key = `${f.baseId}|${normalizeDateKey(f.date)}|${f.period}`;
      if (!m.has(key)) m.set(key, []);
      m.get(key)!.push(f);
    }
    return m;
  }, [freeSlots]);
  const freeIn = (baseId: string, date: string, period: string) =>
    freeBySlot.get(`${baseId}|${date}|${period}`) ?? [];
  ...
  async function openLottery() {
    ...
    const json = await fetchJsonNoStore(`${baseApi}/interns`);
    if (json.success) {
  ...
  async function handleRemoveIntern(id: string, permanent: boolean) {
    try {
      if (permanent) {
        await enviarJson(`${baseApi}/interns`, "PATCH", { userId: id, action: "deactivate" });
  ...
  async function handleReactivateIntern(id: string) {
    try {
      await enviarJson(`${baseApi}/interns`, "PATCH", { userId: id, action: "reactivate" });
      setLotteryInterns((prev) =>
  ...
  async function runLottery() {
    ...
    const res = await enviarJson(`${baseApi}/lottery`, "POST", {
      weekStart, internIds: ids, maxShifts, numWeeks,
    });
  ...
  /* ── Manual allocation ── */
  function openAllocModal(baseId: string, baseCode: string, baseType: string, date: string, period: "DAY" | "NIGHT", isExtra?: boolean, free?: { id: string; abbr: string }) {
    setAllocSlot({ baseId, baseCode, baseType, date, period, free });
    setAllocInternId("");
    setAllocSearch("");
    setAllocMsg("");
  ...
  async function submitAllocation() {
    ...
    setAllocMsg("");
    try {
      const res = allocSlot.free
        ? await enviarJson(`/taximetro/api/extra-offers/${allocSlot.free.id}`, "POST", {
          internId: allocInternId,
          facultyId: effectiveFacultyId,
        })
        : await enviarJson("/taximetro/api/assignments", "POST", {
          internId: allocInternId,
          facultyId: effectiveFacultyId,
  ...
  async function confirmRemoveAssignment() {
    ...
    setRemoveError("");
    try {
      const res = await enviarJson("/taximetro/api/assignments", "PUT", {
        id: removeTarget.id,
        status: "CANCELLED",
        // A nota fica no plantão e é lida por gente: quem removeu importa.
        notes: `Removido da escala ${escopo.tipo === "admin" ? "pela coordenação" : "pelo líder"} em ${formatDateLabel(localDateStr(), { day: "2-digit", month: "2-digit", year: "numeric" })}`,
      });
      const contentType = res.headers.get("content-type") ?? "";
      const json = contentType.includes("application/json") ? await res.json() : null;
      if (json.success) {
  ...
  async function addCruFixed() {
    ...
    setCruFixedMsg("");
    try {
      const res = await enviarJson(`${baseApi}/cru-fixed`, "POST", {
  ...
  async function generateCruWeek() {
    setCruGenMsg("");
    try {
      const res = await enviarJson(`${baseApi}/cru-generate`, "POST", { weekStart });
      const json = await res.json();
      if (json.success) {
      ...
      <section className="space-y-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-xs text-slate-500">
            Cada célula representa a vaga da faculdade nas bases USA. Quando há interno escalado, o nome aparece; quando sobra vaga, a célula fica como vaga disponível.
          </span>
        </div>
                ...
                <Fragment key={`schedule-${base.id}`}>
                  ...
                  {weekDates.map((d) => {
                    ...
                    const periodBlocks = periods.map((period) => {
                      ...
                      const freeHere = freeIn(base.id, d, period);
                      const openCount = Math.max(capacity - periodAssignments.length - releasedHere.length, 0);

                      if (capacity === 0 && periodAssignments.length === 0 && freeHere.length === 0) return null;

                      const periodStyle = getPeriodStyle(period);
                      const containerStyle = capacity === 0 && freeHere.length === 0
                      ...
                      return (
                        <div key={`${base.id}|${d}|${period}`} className={`rounded-lg border p-1.5 ${containerStyle}`}>
                          ...
                          <div className="space-y-1">
                            ...
                            {capacity === 0 && periodAssignments.length > 0 && freeHere.length === 0 && (
            ...
            <div className="min-w-[980px]" style={{ display: "grid", gridTemplateColumns: "140px repeat(7, minmax(118px, 1fr))" }}>
              ...
              {weekDates.map((d) => {
                ...
                return (
                  ...
                  >
                    <div className="space-y-1.5">
                      {cellAssignments.length === 0 && dayCapacity === 0 && freeIn(crlBase.id, d, "DAY").length === 0 && (
            ...
            <div className="min-w-[980px]" style={{ display: "grid", gridTemplateColumns: "140px repeat(7, minmax(118px, 1fr))" }}>
              ...
              {weekDates.map((d) => {
                ...
                const periodBlocks = periods.map((period) => {
                  ...
                  if (capacity === 0 && periodAssignments.length === 0 && freeIn(cruBase.id, d, period).length === 0) return null;
                  ...
                  return (
                    <div key={`${cruBase.id}|${d}|${period}`} className={`rounded-lg border p-1.5 ${containerStyle}`}>
                      ...
                      <div className="space-y-1">
                        ...
                        {freeIn(cruBase.id, d, period).map((f) => (
                              <button
                                key={f.id}
                                type="button"
                                onClick={() => openAllocModal(f.baseId, f.baseCode, f.baseType, d, period, false, { id: f.id, abbr: f.releasedFacultyAbbr })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden">
            <div className="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-4 text-white flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-blue-100">
                  ...
                  {allocSlot.free && ` · vaga livre da ${allocSlot.free.abbr}`}
                </p>
              </div>
            ...
            <div className="px-6 py-4 space-y-3">
              ...
              {!allocSlot.free && <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 space-y-2">
              ...
              {allocSlot.date >= today && !allocSlot.free && (
                <button
                  type="button"
                  disabled={releaseBusy !== ""}
