...
import { GradeDeRemanejamento, useRemanejamento } from "@/components/intern/remanejamento-interno";
...
export function AvisoCoordenacao({ assignmentId, icon }: { assignmentId: string; icon?: ReactNode }) {
  ...
  async function avisar(tipo: string, rotulo: string) {
    ...
    try {
      ...
      if (json.success) await remanejamento.buscar();
    } catch {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        ...
        >
          {remanejamento.carregando ? "Carregando a grade..." : remanejamento.grade ? "Atualizar a grade" : "Ver vaga em outra base"}
        </button>
