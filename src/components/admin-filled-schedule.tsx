...
export function AdminFilledSchedule({ scope = "all" }: { scope?: ScheduleScope }) {
    ...
    /** Sigla da faculdade cujo painel "quem está onde" está aberto (tag de contagem). */
    const [contagemFaculty, setContagemFaculty] = useState<string | null>(null);
                            ...
                            {contagemPlantoes.items.map((item) => {
                                ...
                                return (
                                    <button
                                        key={item.abbr}
                                        type="button"
                                        onClick={() => { setContagemFaculty(item.abbr); void loadUsers(); }}
                                        title={`${item.abbr}: ${item.usa} USA e ${item.cru} CRU ${filterDayKey ? "no dia" : "na semana"} — toque para ver quem está onde`}
                                        className={`inline-flex cursor-pointer items-center gap-1 rounded-full px-2.5 py-1 font-medium transition-all hover:scale-105 active:scale-95 ${facultyTone.pill}`}
                                    >
                                        <span className={`h-2 w-2 rounded-full ${facultyTone.dot}`} />
                                        {item.abbr}
                    ...
                    <div className="flex max-h-[92dvh] w-full max-w-md flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                            <div className="flex min-w-0 items-center gap-2">
                                ...
                                <h2 className="truncate text-sm font-bold text-slate-900">{contagemPainel.abbr} · {formatDayMonth(weekStart)} — {formatDayMonth(weekEnd)}</h2>
                            </div>
