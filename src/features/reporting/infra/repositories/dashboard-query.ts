...
export async function fetchDashboardData(
  ...
  const [facultyRows, baseRows, baseDetailRows, weekDayRows] = await Promise.all([
    ...
    db.execute(sql`
      SELECT b.code AS base_code, b.name AS base_name,
             u.id AS intern_id, u.name AS intern_name, f.abbreviation AS faculty,
  ...
  const [absenceRows, geoRows, totpRows, activeRows, assignmentRows, checkedInRows, completedDetailRows, selfAssignmentRows] = await Promise.all([
    db.execute(sql`
      SELECT u.id AS intern_id, u.name, f.abbreviation AS faculty, b.code AS base_code, a.period
    ...
    `),
    db.execute(sql`
      SELECT u.id AS intern_id, u.name, f.abbreviation AS faculty, b.code AS base_code, ROUND(c.geo_distance_meters::numeric, 0)::text AS distance
    ...
    `),
    db.execute(sql`
      SELECT u.id AS intern_id, u.name, f.abbreviation AS faculty, b.code AS base_code, a.period, a.date
      FROM assignments a JOIN users u ON u.id = a.intern_id JOIN bases b ON b.id = a.base_id JOIN faculties f ON f.id = a.faculty_id
    ...
    `),
    db.execute(sql`
      SELECT a.id, u.id AS intern_id, u.name, f.abbreviation AS faculty, b.code AS base_code, a.period, a.status, a.date, a.notes
      FROM assignments a JOIN users u ON u.id = a.intern_id JOIN bases b ON b.id = a.base_id JOIN faculties f ON f.id = a.faculty_id
  ...
  const toDetail = (row: Record<string, unknown>) => ({
    internId: row.intern_id ? String(row.intern_id) : null,
  ...
  const incidents = [
    ...(geoRows as Record<string, unknown>[]).map((r) => ({
      internId: r.intern_id ? String(r.intern_id) : null,
      name: String(r.name ?? ""),
