      ...
      if (account?.provider === "google") {
        try {
          ...
          const [existing] = await db.select().from(users)
            ...
            .limit(1);
          if (!existing) {
            // Self-registration via Google is disabled. Internos must use the
            // invite link generated by their leader so faculty/turma are bound
            // server-side and cannot be picked manually.
            await logGoogleLoginEvent({
              action: "LOGIN_GOOGLE_FAILED",
              reason: "NO_ACCOUNT",
              email: user.email,
            ...
            return "/taximetro/login?error=NoInvite";
          }
