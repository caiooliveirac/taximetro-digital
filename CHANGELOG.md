...
### Alterado
- **Ficha do interno para o líder** — a aba `/leader/internos` deixou de abrir uma sanfona dentro da linha da tabela: clicar num interno abre a mesma ficha que o coordenador vê (`InternHistorySection`), com velocímetro, casinhas da meta clicáveis, plantões com detalhe de check-in/checkout, ocorrências e trocas, mais os botões de Alocar / Ver escala / Arquivar. ~150 linhas de blocos duplicados saíram da página. **Escopo:** a lista cruza `/api/admin/users` (cadastro) com `/api/leader/interns`, que já recorta faculdade + turma do líder no servidor — líder da UNIFACS turma X vê só os internos não arquivados dessa turma; líder sem turma continua vendo a faculdade inteira, e se a rota falhar a lista degrada para o comportamento antigo em vez de ficar vazia. `/api/compliance?internId=` passou a atender o líder (antes ele recebia a faculdade inteira e a ficha mostrava o cumprimento da primeira pessoa da lista, não do interno aberto); a decisão de escopo virou `resolveComplianceScope`, testada em `tests/compliance-scope.test.ts` — `internId` vem do cliente, a faculdade é fixada no servidor, e pedir interno de outra faculdade devolve vazio. "Abonar falta" continua só para coordenação e preceptor: o botão some para quem o servidor recusaria.
- **Meta da faculdade ficou direta, e virou casinha na tela** — a tela Admin → Faculdades não pede mais meta semanal nem meta de plantões avulsa: só quantos USA, quantos CRU, quantos CRL e quantas horas na rotação. O total de plantões é a soma dos três (`facultyTargetShifts`, em `src/db/schema.ts`), não uma coluna que alguém digita e esquece. Cada plantão exigido virou uma **casinha** (`src/lib/goal-slots.ts`, `tests/goal-slots.test.ts`): plantão realizado, já ocorrido sem checkout, agendado ou falta — e a vaga que ninguém escalou fica com ⚠️. Falta não ocupa vaga (o interno segue devendo o plantão); abono ocupa; extra não conta, mesma regra do compliance. As casinhas aparecem em `/intern` (o interno vê antes do fim da rotação que não vai fechar a carga horária), em Ver Interno do admin (clicáveis, abrem o detalhe do plantão), no histórico do interno do líder e como contagem de "vagas em aberto" no cockpit do líder e nos relatórios. O compliance passou a expor `targetUSATotal`/`targetCRUTotal`/`targetCRLTotal`, `missingUSA`/`missingCRU`/`missingCRL` e `missingSlots`, e deixou de expor as metas semanais e o `belowWeeklyTarget` (o resumo agora traz `belowTypeTarget`). O alarme "atraso sem cobertura" do cockpit e o velocímetro passaram a ratear a meta pela fração da rotação já corrida, em vez de multiplicar meta semanal por semanas. Migração `drizzle/0027_faculty_direct_targets.sql` cria `target_crls_total` e preenche as metas totais a partir das semanais (× 4 semanas de rotação nominal) — **rodar antes de conferir as metas na tela**; sem ela, quem só tinha meta semanal fica com meta zero. As colunas semanais continuam no banco, sem leitor, porque o deploy roda `drizzle-kit push --force`.
