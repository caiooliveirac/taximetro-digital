...
async function main() {
    ...
    // Entrega ao Telegram da administração: dump + relatórios em PDF.
    // Só acontece se TELEGRAM_ADMIN_CHAT_ID estiver configurado — sem ele, este
    // bloco inteiro é no-op e o backup continua exatamente como antes.
    ...
    if (recipients.length === 0 && !s3Config && !adminChatId) {
...
async function notifyTelegramFailure(errorMessage) {
    ...
    // Grupo e admin: o alerta de backup quebrado é o que avisa que a tábua de
    // salvação parou. Mandar para os dois é barato e evita depender de um só.
